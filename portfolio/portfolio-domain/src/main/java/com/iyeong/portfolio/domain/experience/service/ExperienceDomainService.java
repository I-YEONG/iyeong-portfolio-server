...
public class ExperienceDomainService {
    ...
    public PfExperience saveExperience(PfExperience pfExperience) {
        ...
        if (experienceRepository.existsByTitle(pfExperience.getTitle())) {
            throw new BaseException(400, "이미 등록된 활동 이름입니다.");
        }
    ...
    public void deleteExperience(Long experienceId) {
        ...
        if (!experienceRepository.existsById(experienceId)) {
            throw new BaseException(404, "존재하지 않는 활동 번호입니다.");
        }
    ...
    // 수정 (더티 체킹)
    @Transactional
    public void updateExperience(Long id, String title, List<PfExperience.ActivityType> type, String detail, LocalDate startDate, LocalDate endDate, String note) {
        // 기존 데이터 호출
        PfExperience experience = experienceRepository.findById(id)
                .orElseThrow(() -> new BaseException(404, "수정할 활동이 존재하지 않습니다."));
        experience.update(title, type, detail, startDate, endDate, note);
    }
}